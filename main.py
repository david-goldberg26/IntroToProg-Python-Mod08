...
employees = FileProcessor.read_employee_data_from_file(file_name=FILE_NAME)  # Note this is the class name (ignore the warning)
...
while True:
    ...
    if menu_choice == "1":  # Display current data
        ...
        try:
            employees = IO.input_employee_data(employee_data=employees)  # Note this is the class name (ignore the warning)
            IO.output_employee_data(employee_data=employees)
        except Exception as e:
            IO.output_error_messages(e)
        continue
